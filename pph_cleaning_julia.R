...
### Steps missing to get to comorbidity index -----------------------------------------------------------------------------------------------------------------------------------

# Ideally: reproduce the Bateman Maternal Comorbidity Index
# Separate severe from non-severe pre-eclampsia
# Clean a TON of additional comorbidity ICD-10 codes
# Assign age weights
# Assign disease weights
# Put the score into the regression

# An alternative: creating our own comorbidity index based on the predictor variables we already cleaned (theory based approach)
# This will also take a while for me to figure out how to do

# Another alternative: forget comorbidiy adjustments
# Just replicate the primary analyses, and ignore the plot stratified by comorbidity index
# (I'll make a figure with APRDRG_severity just out of curiosity)
# When calculating RRs, you want to make 2 models per characteristic:
# One univariate model (unadjusted RR)
# One multivariate model (adjusted RR) - no interactions in this model
